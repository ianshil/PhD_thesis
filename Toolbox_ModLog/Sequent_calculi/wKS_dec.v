...
(* In this file we show that the applicability of the rules in wKS is decidable. *)
...
Lemma dec_is_PropVar : forall (A : MPropF), (existsT2 (P : V), A = # P) + ((existsT2 (P : V), A = # P) -> False).
Proof.
induction A.
...
Lemma dec_prop_var_in : forall s, (existsT2 (P : V), (In (# P) (fst s)) /\ (In (# P) (snd s))) +
                                      ((existsT2 (P : V), (In (# P) (fst s)) /\ (In (# P) (snd s))) -> False).
Proof.
intro. destruct s.
induction l.
...
Lemma dec_wKS_init_rules : forall (s :Seq) ,
...
Lemma dec_KR_rule : forall (concl :Seq),
          (existsT2 prem, wKRRule [prem] concl) + ((existsT2 prem, wKRRule [prem] concl) -> False).
Proof.
intro concl. destruct concl. destruct (dec_box_in_list l0).
- left. destruct s. exists ((unboxed_list (top_boxes l)), [x]).
  apply in_splitT in i. destruct i. destruct s. subst. apply wKRRule_I.
...
- right. intro. destruct X. inversion w. subst. apply f. exists A. apply in_or_app. right. apply in_eq.
Qed.

Lemma dec_wKS_rules : forall (concl :Seq) ,
          ((existsT2 prems, wKS_rules prems concl) -> False) + (existsT2 prems, wKS_rules prems concl).
Proof.
intro concl. destruct (dec_wKS_init_rules concl).
...
- destruct (dec_ImpR_rule concl).
  ...
  * destruct (dec_ImpL_rule concl).
    ...
    + destruct (dec_KR_rule concl).
      { right. repeat destruct s. exists [x]. apply wKR. assumption. }
      { left. intro. destruct X. inversion w.
