...
- pose (partition_1_element2 Δ2 Δ3 Δ4 Δ5 Δ6 Δ0 Δ1 (Box A) H6). destruct s0.
  + repeat destruct s0. repeat destruct p. subst. assert (E : (Δ0 ++ Box A :: x0) ++ Δ5 ++ Δ4 ++ Δ3 ++ Δ6 = Δ0 ++ Box A :: (x0 ++ Δ5 ++ Δ4 ++ Δ3 ++ Δ6)).
    repeat rewrite <- app_assoc. reflexivity. rewrite E. apply wKRRule_I.
    * assumption.
    * assumption.
  + destruct s0.
...
Theorem wKS_adm_list_exch_L : forall s,
        (wKS_prv s) ->
...
list_exch_L x se ->
derrec wKS_rules  (fun _ : Seq => False)
  se)
in D.
- exact D.
...
- intros ps concl rule. induction rule.
  ...
  * intros ders IH se exch. inversion i. apply derI with (ps:=[]).
    ...
    + assert (Permstose: existsT2 eΓ0 eΓ1, se = (eΓ0 ++ # P :: eΓ1, Δ0 ++ # P :: Δ1)).
      { pose (list_exch_L_permLR exch). subst. destruct s0 with (Γ2:=Γ0) (Γ3:=Γ1) (Δ2:=Δ0) (Δ3:=Δ1)
  ...
  * intros ders IH se exch. inversion i. subst. pose (ImpL_app_list_exchL exch i).
  ...
  (* wKR *)
  * intros ders IH se exch. inversion w. subst. pose (KR_app_list_exchL exch w).
    destruct s0. destruct p. apply derI with (ps:=[x]).
    + apply wKR. assumption.
    + apply dersrec_all. apply dersrec_all in ders. inversion w0.
      inversion w. apply ForallT_inv in IH. apply ForallT_cons.
      { apply IH. subst. assumption. }
      { apply ForallT_nil. }
Qed.

Theorem wKS_adm_list_exch_R : forall s,
        (wKS_prv s) ->
...
- intros ps concl rule. induction rule.
  ...
  * intros ders IH se exch. inversion i. apply derI with (ps:=[]).
    ...
    + assert (Permstose: existsT2 eΔ0 eΔ1, se = (Γ0 ++ # P :: Γ1, eΔ0 ++ # P :: eΔ1)).
      { pose (list_exch_R_permLR exch). subst. destruct s0 with (Γ2:=Γ0) (Γ3:=Γ1) (Δ2:=Δ0) (Δ3:=Δ1)
      (C:=# P) (D:=# P). auto. exists x. assumption. }
      destruct Permstose. repeat destruct s0. subst. apply IdPRule_I.
    + apply dersrec_all in ders. apply dersrec_all. subst. assumption.
  (* BotL *)
  ...
  * intros ders IH se exch. inversion i. subst. pose (ImpL_app_list_exchR exch i).
    ...
    + apply dersrec_all. apply dersrec_all in ders. inversion i0.
      ...
      { apply ForallT_cons. apply f. subst. assumption. apply ForallT_nil. }
  (* wKR *)
  * intros ders IH se exch. inversion w. subst. pose (KR_app_list_exchR exch w).
    destruct s0. destruct p. apply derI with (ps:=[x]).
    + apply wKR. assumption.
    + apply dersrec_all. apply dersrec_all in ders. inversion w0.
      inversion w. apply ForallT_inv in IH. apply ForallT_cons.
...
Theorem wKS_der_list_exch_L : forall hyps s,
        (derrec wKS_rules hyps s) ->
        (forall se, (@list_exch_L s se) ->
        (derrec wKS_rules (ExcClosure hyps) se)).
Proof.
intros hyps s D. apply derrec_all_rect with
(Q:= fun x => forall (se : Seq),
list_exch_L x se ->
derrec wKS_rules  (ExcClosure hyps) se) in D.
- exact D.
...
Theorem wKS_der_list_exch_R : forall hyps s,
        (derrec wKS_rules hyps s) ->
        (forall se, (@list_exch_R s se) ->
        (derrec wKS_rules (ExcClosure hyps) se)).
Proof.
...
Theorem wKS_hpadm_list_exch_R0 : forall (k : nat) s
                                  (D0: wKS_prv s),
        k = (derrec_height D0) ->
        (forall se, (list_exch_R s se) ->
        (existsT2 (D1 : wKS_prv se),
          derrec_height D1 <=k)).
Proof.
...
- intros hei se exch. inversion exch.
  ...
  * inversion X. subst. inversion H5. subst. simpl. simpl in IH.
    pose (KR_app_list_exchR exch X). destruct s. destruct p.
    apply wKR in w0. assert (J0: dersrec_height d = dersrec_height d). reflexivity.
    pose (dersrec_derrec_height d J0). destruct s.
    assert (E: derrec_height x0 < S (dersrec_height d)). lia.
    assert (E1: derrec_height x0 = derrec_height x0). auto.
    ...
    destruct s. pose (dlCons x1 DersNil).
    pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
    (ps:=[x]) (Γ, Δ0 ++ Δ3 ++ Δ2 ++ Δ1 ++ Δ4) w0 d0). exists d1. simpl.
    rewrite dersrec_height_nil. rewrite Nat.max_0_r. lia. reflexivity.
Qed.

Theorem wKS_hpadm_list_exch_L0 : forall (k : nat) s
                                  (D0: wKS_prv s),
        k = (derrec_height D0) ->
        (forall se, (list_exch_L s se) ->
        (existsT2 (D1 : wKS_prv se),
...
- intros hei se exch. inversion exch.
  ...
  * inversion H1. subst. inversion H5. subst. simpl. simpl in IH.
  ...
  (* wKR *)
  * inversion X. subst. inversion H5. subst. simpl. simpl in IH.
    pose (KR_app_list_exchL exch X). destruct s. destruct p.
    apply wKR in w0. assert (J0: dersrec_height d = dersrec_height d). reflexivity.
    pose (dersrec_derrec_height d J0). destruct s.
    assert (E: derrec_height x0 < S (dersrec_height d)). lia.
    assert (E1: derrec_height x0 = derrec_height x0). auto.
    ...
    destruct s. pose (dlCons x1 DersNil).
    pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
    (ps:=[x]) (Γ0 ++ Γ3 ++ Γ2 ++ Γ1 ++ Γ4, Δ0 ++ Box A :: Δ1) w0 d0). exists d1. simpl.
    rewrite dersrec_height_nil. rewrite Nat.max_0_r. lia. reflexivity.
Qed.

Theorem wKS_hpadm_list_exch_L : forall s (D0: wKS_prv s),
        (forall se, (list_exch_L s se) ->
        (existsT2 (D1 : wKS_prv se),
          derrec_height D1 <= derrec_height D0)).
Proof.
intros.
pose (@wKS_hpadm_list_exch_L0 (derrec_height D0) _ D0).
apply s0 ; auto.
Qed.

Theorem wKS_hpadm_list_exch_R : forall s (D0: wKS_prv s),
        (forall se, (list_exch_R s se) ->
        (existsT2 (D1 : wKS_prv se),
          derrec_height D1 <= derrec_height D0)).
Proof.
intros.
pose (@wKS_hpadm_list_exch_R0 (derrec_height D0) _ D0).
apply s0 ; auto.
Qed.

Theorem wKS_adm_list_exch_LR : forall s (D0: wKS_prv s),
        (forall se, ((list_exch_L s se) -> (wKS_prv se)) *
                        ((list_exch_R s se) -> (wKS_prv se))).
Proof.
intros. assert (J1: derrec_height D0 = derrec_height D0). auto. split ; intro.
- pose (@wKS_hpadm_list_exch_L0 (derrec_height D0) s D0 J1 se H). destruct s0 ; auto.
- pose (@wKS_hpadm_list_exch_R0 (derrec_height D0) s D0 J1 se H). destruct s0 ; auto.
Qed.
