...
Theorem derrec_height_False_ge_1 : forall s, forall (D : wKS_prv s), 1 <= derrec_height D.
Proof.
intros s D.
induction D.
- destruct p.
...
Theorem ImpR_ImpL_hpinv : forall (k : nat) concl
        ...
        k = (derrec_height D0) ->
          ...
          existsT2 (D1 : wKS_prv prem),
          derrec_height D1 <= k))) *
          ((forall prem1 prem2, ((ImpLRule [prem1; prem2] concl) ->
          existsT2 (D1 : wKS_prv prem1)
                   (D2 : wKS_prv prem2),
...
pose (strong_inductionT (fun (x:nat) => forall (concl : Seq)
  (D0 : derrec (wKS_rules) (fun _ : Seq => False) concl),
x = derrec_height D0 ->
((forall prem, ((ImpRRule [prem] concl) ->
          existsT2 (D1 : wKS_prv prem),
          derrec_height D1 <= x))) *
...
{ intros prem RA. inversion RA. subst.
  ...
  * inversion H. subst. apply app2_find_hole in H3. destruct H3. repeat destruct s ; destruct p ; subst.
    + inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
      ...
      pose (wKS_hpadm_list_exch_L0 x0 J20 J1). destruct s.
      ...
      pose (wKS_hpadm_list_exch_L0 x1 J21 J2). destruct s. exists x2.
      simpl. lia.
    + destruct x.
      { simpl in e0. inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        pose (wKS_hpadm_list_exch_L0 x J20 J1). destruct s.
        ...
        pose (wKS_hpadm_list_exch_L0 x0 J21 J2). destruct s. exists x1.
        simpl. lia. }
      { inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). destruct s.
        ...
        rewrite H0. clear H0. apply list_exch_LI.
        assert (J20: derrec_height x0 = derrec_height x0). reflexivity.
        pose (wKS_hpadm_list_exch_L0 x0 J20 J1). destruct s. simpl in IH. simpl.
        ...
        assert (existsT2 (x3 : derrec wKS_rules (fun _ : Seq => False)
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (wKS_hpadm_list_exch_R0 x J8 J7). destruct s.
    ...
    pose (wKS_hpadm_list_exch_L0 d2 J41 J40). destruct s. exists x4. simpl in J41.
    simpl in l2. rewrite dersrec_height_nil in l2. lia. reflexivity.
  (* wKR *)
  * inversion X. subst.
    assert (wKRRule [(unboxed_list BΓ , [A0])] (Γ0 ++ Γ1, Δ0 ++ Δ1)).
    ...
    apply in_splitT in H. destruct H. destruct s. rewrite e. apply wKRRule_I ; try assumption.
    simpl. simpl in IH.
    apply wKR in X1.
...
{ intros prem1 prem2 RA. inversion RA. subst.
  ...
  * inversion H. subst. simpl in IH.
    ...
    + assert (ImpLRule [(Γ2 ++ A0 :: Γ1, A :: Δ2 ++ B0 :: Δ3) ; (Γ2 ++ A0 :: B :: Γ1, Δ2 ++ B0 :: Δ3)]
      ...
      apply ImpR in H1.
      pose (dlCons x1 DersNilF). pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
      ...
      apply list_exch_RI. pose (wKS_hpadm_list_exch_R0 x3 J3 J4). destruct s. exists x4.
      ...
      apply ImpR in H2.
      pose (dlCons x2 DersNilF). pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
    ...
    + destruct x0.
      { simpl in e1. subst. assert (ImpLRule [(Γ0 ++ A0 :: Γ1, A :: Δ2 ++ B0 :: Δ3) ; (Γ0 ++ A0 :: B :: Γ1, Δ2 ++ B0 :: Δ3)]
        ...
        apply ImpR in H1.
        pose (dlCons x0 DersNilF). pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
        ...
        assert (A :: Δ0 ++ Δ1 = [] ++ [A] ++ Δ0 ++ [] ++ Δ1). reflexivity. rewrite H2. clear H2.
        assert (Δ0 ++ A :: Δ1 = [] ++ [] ++ Δ0 ++ [A] ++ Δ1). reflexivity. rewrite H2. clear H2.
        apply list_exch_RI. pose (wKS_hpadm_list_exch_R0 x2 J3 J4). destruct s. exists x3.
  ...
  * inversion H. subst.
    ...
    + inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
      ...
      rewrite H0. clear H0. apply list_exch_RI.
      assert (J20: derrec_height x0 = derrec_height x0). reflexivity.
      pose (wKS_hpadm_list_exch_R0 x0 J20 J1). destruct s.
      ...
      pose (wKS_hpadm_list_exch_R0 x2 J21 J2). destruct s. exists x3.
      assert (existsT2 (x4: derrec wKS_rules (fun _ : Seq => False) (Γ2 ++ B0 :: Γ3, Δ0 ++ Δ1)),
    ...
    + destruct x.
      { simpl in e0. inversion e0. simpl. rewrite app_nil_r. subst.
        ...
        rewrite H0. clear H0. apply list_exch_RI.
        assert (J20: derrec_height x = derrec_height x). reflexivity.
        pose (wKS_hpadm_list_exch_R0 x J20 J1). destruct s.
        assert (J2: list_exch_R (Γ2 ++ Γ1, A :: Δ0 ++ Δ1) (Γ2 ++ Γ1, Δ0 ++ A :: Δ1)).
        assert (A :: Δ0 ++ Δ1 = [] ++ [A] ++ Δ0 ++ [] ++ Δ1).
        reflexivity. assert (Δ0 ++ A :: Δ1 = [] ++ [] ++ Δ0 ++ [A] ++ Δ1).
        reflexivity. rewrite H1. rewrite H0. clear H1. clear H0. apply list_exch_RI.
        assert (J21: derrec_height x1 = derrec_height x1). reflexivity.
        pose (wKS_hpadm_list_exch_R0 x1 J21 J2). destruct s. exists x2.
        assert (existsT2 (x3: derrec wKS_rules (fun _ : Seq => False) (Γ2 ++ B :: Γ1, Δ0 ++ Δ1)),
      ...
      { inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        rewrite H0. clear H0. apply list_exch_RI.
        assert (J20: derrec_height x0 = derrec_height x0). reflexivity.
        pose (wKS_hpadm_list_exch_R0 x0 J20 J1). destruct s. simpl in IH. simpl.
        ...
        destruct p.
        assert (existsT2 (x7 : derrec wKS_rules (fun _ : Seq => False)
        ...
        assert (existsT2 (x8 : derrec wKS_rules (fun _ : Seq => False)
    ...
    + destruct x.
      { simpl in e0. inversion e0. simpl. subst.
        ...
        assert (J21: derrec_height x1 = derrec_height x1). reflexivity.
        pose (wKS_hpadm_list_exch_R0 x1 J21 J2). destruct s. exists x2.
        assert (existsT2 (x3: derrec wKS_rules (fun _ : Seq => False) (Γ0 ++ B0 :: Γ3, Δ0 ++ Δ1)),
  ...
  * inversion X. subst. pose (univ_gen_ext_splitR _ _ X0). repeat destruct s. repeat destruct p. subst.
    assert (J0: dersrec_height d = dersrec_height d). reflexivity.
    pose (dersrec_derrec_height d J0). destruct s.
    assert (wKRRule [(unboxed_list (x ++ x0), [A0])] (Γ0 ++ Γ1, Δ0 ++ Δ1)).
    rewrite <- H2. apply wKRRule_I ; try assumption. apply univ_gen_ext_combine.
    ...
    assert (existsT2 (D : derrec wKS_rules (fun _ : Seq => False) (Γ0 ++ Γ1, Δ0 ++ Δ1)),
    ...
    pose (wKS_wkn_L x2 J1 J2). destruct s.
    assert (J3: wkn_R A (Γ0 ++ Γ1, Δ0 ++ Δ1) (Γ0 ++ Γ1, Δ0 ++ A :: Δ1)). apply wkn_RI.
    pose (wKS_wkn_R x2 J1 J3). destruct s. exists x4. exists x3. simpl.
    split ; lia. }
Qed.


Theorem ImpR_inv : forall concl prem, (wKS_prv concl) ->
                                      (ImpRRule [prem] concl) ->
                                      (wKS_prv prem).
Proof.
intros.
...
Theorem ImpL_inv : forall concl prem1 prem2, (wKS_prv concl) ->
