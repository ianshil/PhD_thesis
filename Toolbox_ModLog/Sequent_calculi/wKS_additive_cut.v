...
Theorem wKS_cut_adm_main : forall n k A s Γ0 Γ1 Δ0 Δ1,
                      (n = size A) ->
                      (k = mhd s) ->
                      (s = (Γ0 ++ Γ1, Δ0 ++ Δ1)) ->
                      (wKS_prv (Γ0 ++ Γ1, Δ0 ++ A :: Δ1)) ->
                      (wKS_prv (Γ0 ++ A :: Γ1, Δ0 ++ Δ1)) ->
                      (wKS_prv s).
Proof.
...
assert (DersNilF: dersrec wKS_rules (fun _ : Seq => False) []).
apply dersrec_nil.
assert (DersNilT: dersrec wKS_rules (fun _ : Seq => True) []).
apply dersrec_nil.
  ...
  * destruct s.
    ...
    { inversion H. subst. assert (InT  # P (Γ0 ++ Γ1)). rewrite <- H2. apply InT_or_app.
      ...
      - apply InT_split in i. destruct i.
        ...
        pose (@wKS_hpadm_ctr_L (derrec_height D1) (x ++ # P :: x0 ++ # P :: Γ1, Δ0 ++ Δ1)
      ...
      - apply InT_split in i. destruct i. destruct s. subst.
        ...
        pose (@wKS_hpadm_ctr_L (derrec_height D1) (Γ0 ++ # P :: x ++ # P :: x0, Δ0 ++ Δ1)
        ...
        apply list_exch_LI. pose (wKS_adm_list_exch_L x1 J5). rewrite H7. assumption. }
    (* Right rule is ImpL *)
    { inversion H. subst. assert (InT  # P (Γ0 ++ Γ1)). rewrite <- H2. apply InT_or_app.
      right. apply InT_eq. apply InT_app_or in H0. destruct H0.
      - apply InT_split in i. destruct i.
    ...
    { inversion X3. subst. assert (InT  # P (Γ0 ++ Γ1)). rewrite <- H2. apply InT_or_app.
      ...
      - apply InT_split in i. destruct i. destruct s. subst.
        ...
        apply list_exch_LI. pose (wKS_adm_list_exch_L x1 J5). rewrite H6. assumption. }
...
- inversion H1. subst. apply list_split_form in H3. destruct H3.
  * destruct s.
    + repeat destruct p. subst. inversion X1.
      ...
      { inversion H. subst. inversion X0. inversion X2. subst. clear X4. clear X6. rewrite <- H7 in D1.
        ...
        rewrite H0. clear H0. apply list_exch_LI. pose (d:=wKS_adm_list_exch_L X5 J1). rewrite H2.
        assert (ImpRRule [([] ++ A :: Γ0 ++ Γ1, Δ2 ++ B0 :: Δ3)] ([] ++ Γ0 ++ Γ1, Δ2 ++ A --> B0 :: Δ3)). apply ImpRRule_I.
        simpl in H0.
        assert (J3: wKS_rules [(A :: Γ0 ++ Γ1, Δ2 ++ B0 :: Δ3)] (Γ0 ++ Γ1, Δ2 ++ A --> B0 :: Δ3)).
        apply ImpR ; try intro ; try apply f ; try rewrite <- H7 ; try auto ; try assumption.
        assert (J31: wKS_rules [(A :: Γ0 ++ Γ1, Δ2 ++ B0 :: Δ3)] (Γ0 ++ Γ1, Δ2 ++ A --> B0 :: Δ3)).
        ...
        reflexivity. rewrite H3. clear H3. apply list_exch_RI. pose (d1:=wKS_adm_list_exch_R D0 J8).
      ...
      { inversion H. subst. inversion X0. inversion X2. subst. clear X4. inversion X6. subst. clear X7.
        ...
        - destruct s.
          * repeat destruct p. inversion e0. subst. rewrite H2.
            ...
            rewrite H0. clear H0. apply list_exch_LI. pose (d:=wKS_adm_list_exch_L X3 J1).
            ...
            apply list_exch_RI. pose (d0:=wKS_adm_list_exch_R d J2).
            ...
            reflexivity. pose (wKS_wkn_R X5 J8 J7). destruct s. rewrite <- H2 in d0. pose (d2:=d1 x d0).
            ...
            rewrite H0. clear H0. apply list_exch_LI. pose (d4:=wKS_adm_list_exch_L X4 J30).
            ...
            apply list_exch_LI. pose (d5:=wKS_adm_list_exch_L d4 J40). pose (d3 d2 d5). rewrite <- H2. assumption.
          * repeat destruct s. repeat destruct p. subst. rewrite H2. repeat rewrite <- app_assoc in X5. repeat rewrite <- app_assoc in X4.
            ...
            pose (d:=wKS_adm_list_exch_R D0 J1).
            ...
            assert (J3: wKS_rules [(Γ0 ++ x0 ++ Γ5, Δ2 ++ A :: Δ3); (Γ0 ++ x0 ++ B0 :: Γ5, Δ2 ++ Δ3)] (Γ0 ++ x0 ++ A --> B0 :: Γ5, Δ2 ++ Δ3)).
        ...
        - repeat destruct s. repeat destruct p. subst. rewrite H2. rewrite <- app_assoc. rewrite <- app_assoc in D0.
          ...
          assert (J3: wKS_rules [(Γ4 ++ x ++ Γ1, Δ2 ++ A :: Δ3); (Γ4 ++ B0 :: x ++ Γ1, Δ2 ++ Δ3)] (Γ4 ++ A --> B0 :: x ++ Γ1, Δ2 ++ Δ3)).
          apply ImpL ; try intro ; try apply f ; try rewrite <- H7 ; try repeat rewrite <- app_assoc ; try auto ; try assumption.
      ...
      { inversion X3. subst. rewrite H2.
        assert (wKRRule [(unboxed_list BΓ, [A])] (Γ0 ++ Γ1, Δ2 ++ Box A :: Δ3)).
        apply wKRRule_I ; try assumption.
    ...
    + repeat destruct s. repeat destruct p. subst.
      ...
      apply list_exch_LI. pose (d:=wKS_adm_list_exch_L D1 J5).
      ...
      assert (wKS_rules [(Γ2 ++ A0 :: Γ3, Δ0 ++ x0 ++ B :: Δ3)] (Γ2 ++ Γ3, Δ0 ++ x0 ++ A0 --> B :: Δ3)).
  ...
  * repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc. repeat rewrite <- app_assoc in D1.
    ...
    apply ImpRRule_I.
    assert (wKS_rules [(Γ2 ++ A0 :: Γ3, Δ2 ++ (B :: x) ++ Δ1)] (Γ2 ++ Γ3, Δ2 ++ (A0 --> B :: x) ++ Δ1)).
...
- inversion H1. subst. inversion X0. inversion X4. subst. clear X6. clear X4.
  ...
  assert (A :: Γ0 ++ Γ1 = [] ++ [A] ++ Γ0 ++ [] ++ Γ1). reflexivity. rewrite H. clear H.
  apply list_exch_LI. pose (d:=wKS_adm_list_exch_L D1 J5). rewrite H3 in X5.
  ...
  apply list_exch_RI. pose (d0:=wKS_adm_list_exch_R X3 J40).
  ...
  apply ImpLRule_I. simpl in H0.
  assert (wKS_rules [(Γ2 ++ Γ3, A0 :: Δ0 ++ Δ1); (Γ2 ++ B :: Γ3, Δ0 ++ Δ1)] (Γ2 ++ A0 --> B :: Γ3, Δ0 ++ Δ1)).
  ...
  * destruct s.
    + repeat destruct p. subst. inversion X1.
      ...
      { inversion H. subst. inversion X2. subst. clear X6. rewrite <- H6 in D1.
        ...
        rewrite H0. clear H0. apply list_exch_LI. pose (d:=wKS_adm_list_exch_L X5 J1).
        assert (ImpRRule [([] ++ A :: Γ0 ++ Γ1, Δ2 ++ B :: Δ3)] ([] ++ Γ0 ++ Γ1, Δ2 ++ A --> B :: Δ3)). apply ImpRRule_I.
        simpl in H0.
        assert (J3: wKS_rules [(A :: Γ0 ++ Γ1, Δ2 ++ B :: Δ3)] (Γ0 ++ Γ1, Δ2 ++ A --> B :: Δ3)).
        apply ImpR ; try intro ; try apply f ; try rewrite <- H6 ; try auto ; try assumption.
        assert (J31: wKS_rules [(A :: Γ0 ++ Γ1, Δ2 ++ B :: Δ3)] (Γ0 ++ Γ1, Δ2 ++ A --> B :: Δ3)).
        apply ImpR ; try assumption.
        ...
        reflexivity. rewrite H2. clear H2. apply list_exch_RI. pose (d1:=wKS_adm_list_exch_R D0 J8).
      ...
      { inversion H. subst. apply list_split_form in H5. destruct H5.
        - destruct s.
          ...
          + repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc in H. repeat rewrite <- app_assoc in X2.
            ...
            apply list_exch_RI. pose (d:=wKS_adm_list_exch_R D0 J2).
            ...
            assert (J3: wKS_rules [(Γ0 ++ x0 ++ Γ3, Δ2 ++ A :: Δ3); (Γ0 ++ x0 ++ B :: Γ3, Δ2 ++ Δ3)]
            (Γ0 ++ x0 ++ A --> B :: Γ3, Δ2 ++ Δ3)).
            apply ImpL ; try intro ; try apply f ; try rewrite <- app_assoc ;
            try rewrite <- H6 ; try auto ; try assumption.
            assert (J30: wKS_rules [(Γ0 ++ x0 ++ Γ3, Δ2 ++ A :: Δ3); (Γ0 ++ x0 ++ B :: Γ3, Δ2 ++ Δ3)]
        ...
        - repeat destruct s. repeat destruct p. subst. rewrite <- app_assoc in D0.
          ...
          assert (J3: wKS_rules [(Γ2 ++ x ++ Γ1, Δ2 ++ A :: Δ3); (Γ2 ++ B :: x ++ Γ1, Δ2 ++ Δ3)]
          (Γ2 ++ (A --> B :: x) ++ Γ1, Δ2 ++ Δ3)).
          apply ImpL ; try intro ; try apply f ; try rewrite <- app_assoc ;
          try rewrite <- H6 ; try auto ; try assumption.
          assert (J30: wKS_rules [(Γ2 ++ x ++ Γ1, Δ2 ++ A :: Δ3); (Γ2 ++ B :: x ++ Γ1, Δ2 ++ Δ3)]
      ...
      { inversion X5. subst. inversion X0. subst. clear X8. inversion X2. subst. clear X9.
        ...
        - subst.
          ...
          pose (wKS_wkn_R _ J10 H). destruct s. clear J10. clear H. clear l0.
          ...
          pose (d:=wKS_adm_list_exch_L X8 J5). simpl in d. assert (x1 ++ l = x ++ x0).
          ...
          assert (wKRRule [(unboxed_list (x ++ x0), [A])] (Γ0 ++ Γ1, Δ2 ++ Box A :: Δ3)).
          apply wKRRule_I.
          assumption. apply univ_gen_ext_combine ; auto.
          assert (wKS_rules [(unboxed_list (x ++ x0), [A])] (Γ0 ++ Γ1, Δ2 ++ Box A :: Δ3)).
          apply wKR. assumption.
          pose (dlCons d1 DersNilF).
          pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
          (ps:=[(unboxed_list (x ++ x0), [A])]) (Γ0 ++ Γ1, Δ2 ++ Box A :: Δ3) X10 d2). auto.
        - exfalso. apply H2. exists A0. reflexivity. }
    + repeat destruct s. repeat destruct p. subst.
      assert (wKRRule [(unboxed_list BΓ, [A0])] (Γ0 ++ Γ1, (Δ0 ++ x0) ++ Box A0 :: Δ3)).
      apply wKRRule_I ; try assumption. repeat rewrite <- app_assoc in X5.
      assert (wKS_rules [(unboxed_list BΓ , [A0])] (Γ0 ++ Γ1, Δ0 ++ x0 ++ Box A0 :: Δ3)).
      apply wKR. assumption.
      pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
      (ps:=[(unboxed_list BΓ, [A0])]) (Γ0 ++ Γ1, Δ0 ++ x0 ++ Box A0 :: Δ3) X6 X0). assumption.
  * repeat destruct s. repeat destruct p. subst. rewrite <- app_assoc.
    assert (wKRRule [(unboxed_list BΓ, [A0])] (Γ0 ++ Γ1, Δ2 ++ (Box A0 :: x) ++ Δ1)).
    apply wKRRule_I ; try assumption.
    assert (wKS_rules [(unboxed_list BΓ, [A0])] (Γ0 ++ Γ1, Δ2 ++ (Box A0 :: x) ++ Δ1)).
    apply wKR. assumption.
    pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
    (ps:=[(unboxed_list BΓ, [A0])]) (Γ0 ++ Γ1, Δ2 ++ (Box A0 :: x) ++ Δ1) X6 X0). assumption.
Qed.

Theorem wKS_cut_adm : forall A Γ0 Γ1 Δ0 Δ1,
                      (wKS_prv (Γ0 ++ Γ1, Δ0 ++ A :: Δ1)) ->
                      (wKS_prv (Γ0 ++ A :: Γ1, Δ0 ++ Δ1)) ->
                      (wKS_prv (Γ0 ++ Γ1, Δ0 ++ Δ1)).
Proof.
...
pose (@wKS_cut_adm_main (size A) (mhd (Γ0 ++ Γ1, Δ0 ++ Δ1)) A
