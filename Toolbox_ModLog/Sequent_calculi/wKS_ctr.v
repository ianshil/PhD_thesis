...
Lemma wKR_app_ctr_L : forall s sc A ps,
  (@ctr_L A s sc) ->
  (wKRRule [ps] s) ->
    ((wKRRule [ps] sc) +
     (existsT2 psc, (wKRRule [psc] sc) * (@ctr_L (unBox_formula A) ps psc))).
Proof.
...
Lemma wKR_app_ctr_R : forall s sc A ps,
  (@ctr_R A s sc) -> (wKRRule [ps] s) -> (wKRRule [ps] sc).
Proof.
intros s sc A ps ctr RA. inversion RA. inversion ctr. rewrite <- H1 in H2.
inversion H2. apply app2_find_hole in H6. destruct H6. destruct s0.
  + destruct s0.
    * destruct p. inversion e0. apply wKRRule_I. assumption. assumption.
    * destruct p. subst. destruct x.
      { rewrite app_nil_l in e0. inversion e0. subst. apply wKRRule_I.
        assumption. assumption. }
      { inversion e0. subst. apply app2_find_hole in H3. destruct H3. destruct s.
        - destruct s.
          + destruct p. inversion e1. apply wKRRule_I. assumption. assumption.
          + destruct p. subst. destruct x0.
            * rewrite app_nil_l in e1. inversion e1. subst. apply wKRRule_I. assumption.
              assumption.
            * inversion e1. subst. assert (E: Δ2 ++ m0 :: Δ3 ++ x0 ++ Box A0 :: Δ1 = (Δ2 ++ m0 :: Δ3 ++ x0) ++ Box A0 :: Δ1).
              repeat rewrite <- app_assoc. simpl. repeat rewrite <- app_assoc. reflexivity. rewrite E. apply wKRRule_I. assumption.
              assumption.
        - destruct p. subst. destruct x0.
          * rewrite app_nil_l in e1. inversion e1. subst. apply wKRRule_I. assumption. assumption.
          * inversion e1. subst. assert (E: Δ2 ++ m :: (x ++ Box A0 :: x0) ++ Δ4 = (Δ2 ++ m :: x) ++ Box A0 :: x0 ++ Δ4).
            repeat rewrite <- app_assoc. reflexivity. rewrite E. apply wKRRule_I. assumption. assumption. }
  + destruct p. subst. destruct x.
    * rewrite app_nil_l in e0. rewrite app_nil_r. inversion e0. subst. apply wKRRule_I. assumption. assumption.
    * inversion e0. rewrite <- app_assoc. simpl. apply wKRRule_I. assumption. assumption.
Qed.

(* Now we can prove that contraction rules are height-preserving admissible. *)

Theorem wKS_hpadm_ctr_LR0 : forall (k : nat) s
        (D0 : wKS_prv s),
        k = (derrec_height D0) ->
          (forall sc A, (((@ctr_L A s sc) + (@ctr_R A s sc)) ->
          existsT2 (D1 : wKS_prv sc),
          derrec_height D1 <= k)).
Proof.
...
{ inversion ctr. assert (DersNil: dersrec wKS_rules (fun _ : Seq => False) []).
  apply dersrec_nil. inversion w.
  (* IdP *)
  ...
  * inversion H1. subst. inversion H5. subst. pose (ImpL_app_ctr_L ctr H1). destruct s.
  ...
  (* wKR *)
  * inversion X. rewrite <- H4 in X. pose (wKR_app_ctr_L ctr X). destruct s.
    { apply wKR in w0. pose (derI (rules:=wKS_rules)
      (prems:=fun _ : Seq => False) (ps:=[(unboxed_list BΓ, [A0])]) 
      sc w0). subst. pose (d0 d). exists d1. simpl. reflexivity. }
    { repeat destruct s. repeat destruct p. apply wKR in w0.
      ...
      destruct s.
      pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
      (ps:=[x]) (Γ0 ++ A :: Γ1 ++ Γ2, Δ) w0). subst. simpl.
...
{ inversion ctr. assert (DersNil: dersrec wKS_rules (fun _ : Seq => False) []).
  ...
  * inversion H1. subst. inversion H5. subst. pose (ImpL_app_ctr_R ctr H1). destruct s.
    ...
    assumption. assumption. reflexivity. reflexivity. reflexivity.
  (* wKR *)
  * inversion X. rewrite <- H4 in X. pose (wKR_app_ctr_R ctr X).
    apply wKR in w0.
    pose (derI (rules:=wKS_rules) (prems:=fun _ : Seq => False)
    (ps:=[(unboxed_list BΓ, [A0])]) sc w0). subst. simpl. pose (d0 d). exists d1.
    simpl. apply Le.le_n_S. left. }
Qed.

Theorem wKS_hpadm_ctr_LR : forall s (D0 : wKS_prv s),
          (forall sc A, ((ctr_L A s sc) + (ctr_R A s sc)) ->
          existsT2 (D1 : wKS_prv sc),
...
pose (@wKS_hpadm_ctr_LR0 (derrec_height D0) _ D0 J0 _ _ H).
auto.
Qed.

Theorem wKS_hpadm_ctr_L : forall (k : nat) s
        (D0 : wKS_prv s),
        k = (derrec_height D0) ->
...
pose (@wKS_hpadm_ctr_LR0 (derrec_height D0) s
D0 H1 sc A H3). destruct s0. exists x. lia.
Qed.

Theorem wKS_hpadm_ctr_R : forall (k : nat) s
        (D0 : wKS_prv s),
        k = (derrec_height D0) ->
...
Theorem wKS_hpadm_list_ctr_L : forall Γ1 (k : nat) Γ0 Γ2 Γ3 Δ
        (D0 : wKS_prv (Γ0 ++ Γ1 ++ Γ2 ++ Γ1 ++ Γ3,Δ)),
        k = (derrec_height D0) ->
          existsT2 (D1 : wKS_prv (Γ0 ++ Γ1 ++ Γ2 ++ Γ3,Δ)),
          derrec_height D1 <= k.
Proof.
induction Γ1.
...
- intros. assert (H0: derrec_height D0 = derrec_height D0). reflexivity.
  ...
  pose (@wKS_hpadm_ctr_LR0 (derrec_height D0) (Γ0 ++ (a :: Γ1) ++ Γ2 ++ (a :: Γ1) ++ Γ3, Δ)
...
Theorem wKS_hpadm_list_ctr_R : forall Δ1 (k : nat) Γ Δ0 Δ2 Δ3
        (D0 : wKS_prv (Γ,Δ0 ++ Δ1 ++ Δ2 ++ Δ1 ++ Δ3)),
        k = (derrec_height D0) ->
          existsT2 (D1 : wKS_prv (Γ,Δ0 ++ Δ1 ++ Δ2 ++ Δ3)),
          derrec_height D1 <= k.
Proof.
induction Δ1.
- intros. exists D0. rewrite H. auto.
- intros. assert (H0: derrec_height D0 = derrec_height D0). reflexivity.
  ...
  pose (@wKS_hpadm_ctr_LR0 (derrec_height D0) (Γ, Δ0 ++ (a :: Δ1) ++ Δ2 ++ (a :: Δ1) ++ Δ3)
