...
pose (add_remove_list_preserve_NoDup n0 n). apply remove_list_preserv_NoDup.
assumption.
Qed.
...
Definition term_meas (s : Seq) : nat * nat :=
  pair (length (usable_boxes s)) (n_imp_subformS s).
...
(* Third, we need to prove that if no PSGLS rule is applicable to a sequent s,
   then its derivations have a height equal to 0. *)

Theorem no_PSGLS_rule_applic : forall s, (forall ps, (PSGLS_rules ps s) -> False) ->
                                 forall (D : PSGLS_drv s), derrec_height D = 0.
Proof.
...
Theorem GLR_applic_reduces_measure : forall prem concl,
                ...
                (length (usable_boxes prem) < length (usable_boxes concl)).
Proof.
apply GLR_applic_less_usable_boxes.
Qed.
