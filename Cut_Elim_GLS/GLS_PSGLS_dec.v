...
(* In this file we show that the applicability of the rules in GLS and PSGLS is decidable. *)
...
- destruct IHl.
  ...
  * destruct (dec_is_atomicT a).
    + destruct (In_dec l0 a).
      { inversion i. left. subst. destruct H. exists x. simpl. split. auto. subst. assumption.
...
Lemma dec_PSGLS_rules : forall (concl :Seq) ,
          ((existsT2 prems, PSGLS_rules prems concl) -> False) + (existsT2 prems, PSGLS_rules prems concl).
Proof.
intro concl. destruct (dec_init_rules concl).
- right. repeat destruct s.
  * exists nil. apply PSIdP. assumption.
  * exists nil. apply PSIdB. assumption.
  * exists nil. apply PSBotL. assumption.
- destruct (dec_ImpR_rule concl).
  * right. destruct s. exists [x]. apply PSImpR. assumption.
  * destruct (dec_ImpL_rule concl).
    + right. repeat destruct s. exists [x; x0]. apply PSImpL. assumption.
    + destruct (dec_GLR_rule concl).
      { right. repeat destruct s. exists [x]. apply PSGLR. intro. apply f. auto. intro. apply f. auto. intro. apply f. auto.
        assumption. }
