...
Fixpoint ksat u (rho : nat -> domain) (phi : form) : Prop :=
      ...
      | bin Excl psi chi => exists v, (ksat v rho psi) /\ (reachable v u) /\ ((ksat v rho chi) -> False)
    ...
    Proof.
      ...
      destruct H0. destruct H0. destruct H1. exists x. repeat split ; auto. eauto using mon_P, reach_tran.
      destruct H0. exists x. eauto using mon_P, reach_tran.
    Qed.
...
Ltac comp := repeat (progress (cbn in *; autounfold in *)).
    ...
    Proof.
      ...
      - destruct b; split.
        ...
        * intros H v Hv Hv' ; now apply (IHphi2 v rho xi Hext), (H _ Hv), (IHphi1 v rho xi Hext).
        * intro H. destruct H. exists x. destruct H. destruct H0. repeat split ; auto. apply (IHphi1 x rho xi Hext) ; auto.
          intro. apply H1. apply (IHphi2 x rho xi Hext) ; auto.
        * intro H. destruct H. exists x. symmetry in Hext. destruct H. destruct H0. repeat split ; auto. apply (IHphi1 x xi rho Hext) ; auto.
          intro. apply H1. apply (IHphi2 x xi rho Hext) ; auto.
...
Definition loc_conseq Γ Δ :=
  forall D (M : kmodel D) u rho,
     (forall A, In _ Γ A -> (@ksat D M u rho A)) ->
     (exists B, (In _ Δ B) /\ (@ksat D M u rho B)).

Definition glob_conseq Γ Δ :=
    forall D (M : kmodel D) rho,
    (forall A, (In _ Γ A) -> (forall u,(@ksat D M u rho A))) ->
    (forall u, (exists B, (In _ Δ B) /\ (@ksat D M u rho B))).
